	...
	<div class="container">
		<div class="row">
			...
			<select name="colname" class="input-sm">
				<option value="type" ${colname eq "type" ? "selected" : "" }>음식종류</option>
				<option value="address" ${colname eq "address" ? "selected" : "" }>주소</option>
				<option value="name" ${colname eq "name" ? "selected" : "" }>업체명</option>
			</select>
			...
			<div class="text-center">
				<ul class="pagination">
					...
					<c:forEach var="i" begin="${startPage }" end="${endPage }">
				  		<li ${curpage eq i ? "class=active" : "" }><a href="../food/find.do?page=${i }&colname=${colname }&ss=${ss }">${i }</a></li>
				  	</c:forEach>
